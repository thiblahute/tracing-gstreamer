...
impl<'a> SpanBuilder<'a> {
    ...
    fn build(self, tracer: &TracingTracerPriv) {
        ...
        if let Some(element) = self.element {
            ...
            let message_type_name = self.message.map(|m| crate::message_name(m));
...
impl TracerImpl for TracingTracerPriv {
    ...
    fn element_post_message_post(&self, ts: u64, element: &gstreamer::Element, success: bool) {
        self.pop_span();
    }
}
